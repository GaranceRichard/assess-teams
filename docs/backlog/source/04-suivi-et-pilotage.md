...
Cet Epic applique les [concepts métier transverses](00-concepts-transverses.md). L’organisation de toute vue, comparaison, évolution ou tendance est celle de l’équipe source ; aucune donnée d’une autre organisation n’est consultée ou agrégée implicitement.

### FEAT-024 — Parcourir l’historique des évaluations

- **Intention métier :** restituer la trajectoire d’une équipe dans le temps.
- **Acteurs concernés :** `Coach` autorisé par son affectation, `Admin`, dans leur périmètre applicable.
...
Cet Epic applique les [concepts métier transverses](00-concepts-transverses.md). Chaque paramètre, vue et indicateur expose son organisation et n’agrège jamais plusieurs organisations sans règle explicitement arbitrée. **À arbitrer :** paramètres strictement propres à une organisation ou partageables, et règles d’une éventuelle vue multi-organisation.
