...
- **Décisions produit bloquantes :** `ARB-ORG-005` et `ARB-ORG-009`. Aucun de ces choix ne doit être déduit par l’implémentation ; `TEAM-001` n’est pas prêt à implémenter.
...
- **User story :** en tant que `Viewer` authentifié, je veux consulter les équipes actives de mon périmètre organisationnel afin d’identifier celles que je peux suivre.
...
- **User story :** en tant que `Viewer` authentifié, je veux consulter une équipe précise de mon périmètre organisationnel afin de connaître son identité et son état.
